    ...
    <div className="max-w-4xl">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-3 items-center">
          ...
          <Link href="/todos/new" className="bg-accent text-white px-4 py-2 rounded-lg text-sm hover:bg-accent-hover transition-colors">
            + Add To-Do
          </Link>
        </div>
      </div>
              ...
              {pending.map((t) => (
                <div key={t.id} className="flex items-center gap-4 px-5 py-3 border-b border-border last:border-0 hover:bg-gray-50/50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {t.dueDate && (
                      <p className={`text-xs mt-0.5 ${t.dueDate < new Date().toISOString().split("T")[0] ? "text-danger font-medium" : "text-muted"}`}>
                        Due {new Date(t.dueDate + "T00:00:00").toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}
                      </p>
                    )}
                  </div>
                ...
                {completed.map((t) => (
                  <div key={t.id} className="flex items-center gap-4 px-5 py-3 border-b border-border last:border-0">
                    ...
                    <form action={toggleTodo.bind(null, t.id, false)}>
                      <button type="submit" className="w-5 h-5 rounded border-2 border-success bg-success/20 flex-shrink-0 flex items-center justify-center text-success text-xs" title="Mark as pending">&#10003;</button>
                    </form>
                    <div className="flex-1 min-w-0">
